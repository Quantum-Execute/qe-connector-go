...
> - `pageSize` 最大 100，超过会返回错误；
> - 母单状态枚举：`NEW`、`WAITING`、`PROCESSING`、`PAUSED`、`CANCELLED`、`COMPLETED`、`COMPLETED_WITHTAIL`、`REJECTED`、`EXPIRED`。

以下接口和字段表属于 V1 SDK；V2 strategy-api 详见下文 “V2 strategy-api 接口” 章节。
...
| executionDurationSeconds | int32     | 否 | 执行时长（秒），所有策略（TWAP-1 / POV）均可使用。当提供此字段且>0时，优先使用此字段。必须大于10秒                                                                                                                                                     |
| **TWAP/VWAP 算法参数** |
...
| ├─ commission          | map[string]string | 手续费，key 为手续费单位，value 为数量；无手续费时为空                                                                                                                    |
...
| ├─ executionDurationSeconds | int32   | 执行时长（秒，所有策略均可使用；当提供且>0时优先使用；必须>10秒）                                                                                                                         |
...
| orderNotional | float64 | 否 | 订单金额（USDT）。与 totalQuantity 二选一，且只能修改下单时使用的字段（若下单时使用 orderNotional 则只能改此字段） |
| totalQuantity | float64 | 否 | 要交易的总数量。与 orderNotional 二选一，且只能修改下单时使用的字段（若下单时使用 totalQuantity 则只能改此字段） |
...
| `client.NewGetMasterOrderDetailV2Service()` | GET | `/strategy-api/user/trading/v2/master-orders/{masterOrderId}` | 母单详情（V2 字段） |
| `client.NewGetMasterOrderDetailByClientOrderIdV2Service()` | GET | `/strategy-api/user/trading/v2/master-orders/by-client-order-id/{clientOrderId}` | 通过 `clientOrderId` 查母单 |
| `client.NewGetOrderFillsV2Service()` | GET | `/strategy-api/user/trading/v2/order-fills` | 子单 / 成交列表（V2 字段） |
| `client.NewCancelMasterOrderV2Service()` | PUT | `/strategy-api/user/trading/v2/master-orders/{masterOrderId}/cancel` | 取消母单 |
| `client.NewPauseMasterOrderV2Service()` | PUT | `/strategy-api/user/trading/v2/master-orders/{masterOrderId}/pause` | 暂停母单 |
| `client.NewResumeMasterOrderV2Service()` | PUT | `/strategy-api/user/trading/v2/master-orders/{masterOrderId}/resume` | 恢复母单 |
| `client.NewUpdateMasterOrderParamsV2Service()` | PUT | `/strategy-api/user/trading/v2/master-orders/{masterOrderId}/update` | 修改运行中母单参数 |
...
func main() {
	...
	for _, o := range list.Items {
		...
		log.Printf("- %s account=%s %s %s/%s side=%s status=%s cumFilled=%s/%s avg=%s",
			o.MasterOrderId, o.TradingAccount, o.Algorithm, o.BaseCurrency, o.QuoteCurrency,
			o.Side, o.Status, cumFilledQty, totalQuantity, avgFilledPrice)
	}
